...
// THIS FILE WAS AUTOGENERATED USING {script_name}
...
"""
    elements = str()
    element_template = '  {{{shift_char}, \"\\u{utf_char}\"}}{separator} // {comment}{newline}'
    for (shift_char, utf_char, comment) in table[:-1]:
        elements += element_template.format(shift_char=shift_char,
                                            utf_char=utf_char.replace('0x', ''),
                                            comment=comment,
                                            separator=',',
                                            newline='\n')

    # Last element needs to be slightly different
    shift_char, utf_char, comment = table[-1]
    elements += element_template.format(shift_char=shift_char,
                                        utf_char=utf_char.replace('0x', ''),
                                        comment=comment,
                                        separator=' ',
    ...
    return template.format(script_name=os.path.basename(__file__),
                           elements=elements)
